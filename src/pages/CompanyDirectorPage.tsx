...
const CompanyDirectorPage = () => {
  const [currentTaxYear, setCurrentTaxYear] = useState(new Date().getFullYear());
  const [searchQuery, setSearchQuery] = useState('');
  const [filteredDeadlines, setFilteredDeadlines] = useState(getTaxDeadlines('company-director'));
  const { toast } = useToast();

  const deadlines = getTaxDeadlines('company-director');
  ...
  useEffect(() => {
    toast({
      title: "Company Director Profile Loaded",
      description: "Showing deadlines specific to company directors and limited companies.",
    });
  }, []);
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-4 mb-4">
            ...
            <div className="text-left">
              <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Company Director</h1>
              <p className="text-xl text-gray-600 dark:text-gray-300">Tax calendar for limited companies & directors</p>
            </div>
          </div>
          <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
            Your personalized tax calendar showing corporation tax deadlines, PAYE obligations, and company filing requirements.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="bg-white/95 backdrop-blur-sm border-0 shadow-lg">
            <CardContent className="p-6 text-center">
              <div className="text-3xl font-bold text-green-600 mb-2">{deadlines.length}</div>
        ...
        <Card className="bg-white/95 backdrop-blur-sm border-0 shadow-lg">
          <CardHeader>
            <CardTitle className="flex items-center gap-3">
              ...
              All Company Director Tax Deadlines
            </CardTitle>
          </CardHeader>
          <CardContent>
...
export default CompanyDirectorPage;
