...
import { UserCog, Calendar, AlertTriangle } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import DeadlineCard from '@/components/DeadlineCard';
import SearchFilterBar from '@/components/SearchFilterBar';
import TaxYearSelector from '@/components/TaxYearSelector';

const BothProfilesPage = () => {
  const [currentTaxYear, setCurrentTaxYear] = useState(new Date().getFullYear());
  const [searchQuery, setSearchQuery] = useState('');
  const [filteredDeadlines, setFilteredDeadlines] = useState(getTaxDeadlines('both'));
  const { toast } = useToast();

  const deadlines = getTaxDeadlines('both');
  ...
  useEffect(() => {
    toast({
      title: "Both Profiles Loaded",
      description: "Showing all deadlines for both self-employed and company director activities.",
    });
  }, []);
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-4 mb-4">
            ...
            <div className="text-left">
              <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Both Profiles</h1>
              <p className="text-xl text-gray-600 dark:text-gray-300">Complete tax calendar for self-employed + company director</p>
            </div>
          </div>
          <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
            Your comprehensive tax calendar showing all deadlines for both self-employed work and company director responsibilities.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="bg-white/95 backdrop-blur-sm border-0 shadow-lg">
            <CardContent className="p-6 text-center">
              <div className="text-3xl font-bold text-purple-600 mb-2">{deadlines.length}</div>
        ...
        <Card className="bg-white/95 backdrop-blur-sm border-0 shadow-lg">
          <CardHeader>
            <CardTitle className="flex items-center gap-3">
              ...
              All Tax Deadlines (Self-Employed + Company Director)
            </CardTitle>
          </CardHeader>
          <CardContent>
...
export default BothProfilesPage;
